...
The workspace root is a launcher, not a working directory. It stays on the `main` branch at all times. All real work happens in self-contained session folders that branch off for each piece of work. This separation means the root is always clean, always ready to launch a new session, and never blocked by in-progress work.
...
**Personal** is the individual layer. It includes local-only files, personal settings, and the workspace scratchpad. This layer is gitignored — it exists only on your machine.
...
├─────────────────────────────────────┤
│  Personal (individual, gitignored)  │  local-only-* files, workspace-scratchpad/,
│                                     │  settings.local.json
└─────────────────────────────────────┘
```
...
├── repos/                      # Source clones of project repositories (gitignored)
│   ├── my-app/                 # Cloned project repo (stays on default branch)
│   └── my-api/                 # Another project repo
├── work-sessions/              # One folder per active or paused work session
│   └── fix-auth/               # A single session — self-contained
│       ├── workspace/          # Workspace worktree for this session
│       │   └── repos/          # Real directory with nested project worktrees
...
**repos/** holds source clones of your project repositories. These are cloned here during setup and stay on their default branch — they are never disturbed by work sessions. The whole directory is gitignored: the workspace tracks configuration, not code.

**work-sessions/** holds one folder per active or paused session. Each session folder is self-contained: it has its own workspace worktree (with nested project worktrees inside it), plus a tracked session tracker and any specs or plans for that session. Worktrees are gitignored, but the tracker, specs, and plans are tracked so durable session thinking travels across machines via the workspace branch. [Chapter 2](02-work-sessions.md) covers the session lifecycle in detail.
...
**workspace-scratchpad/** is for disposable files — the session log, hook debug output, temporary pointers. If you wouldn't care about losing it, it goes here. This directory is gitignored and lazy-created.

**.claude/** holds all the configuration that makes the workspace work — rules Claude follows, skills for workflows, hooks for automation, scripts for mechanical sequences, the shared library helpers, and agent definitions for subagent dispatch. These are tracked in git so the whole team shares the same configuration.
...
The workspace relies on naming conventions rather than configuration files or runtime logic. A file named `design-auth-system.md` is a spec. A file named `local-only-notes.md` is personal. A file ending in `.md.skip` is available but inactive. A directory named `work-sessions/fix-auth/` is the self-contained folder for the `fix-auth` session.
...
- The workspace root stays on `main`. All work happens inside self-contained folders under `work-sessions/`.
