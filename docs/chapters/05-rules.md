...
**workspace-structure.md** — Documents the directory layout, shared context levels, naming conventions, and structural rules (root stays on main, only local-only and scratchpad are writable from root). Includes a mandatory override for spec/plan locations — specs go in the project worktree root as `design-{topic}.md` and plans as `plan-{topic}.md`, explicitly overriding any default paths from external skills.
