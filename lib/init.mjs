...
export async function initWorkspace(targetDir) {
  ...
  // Write or append CLAUDE.md with @ imports for mandatory rules
  const imports = [
  ...
  if (!existsSync(claudeMdPath)) {
    writeFileSync(claudeMdPath, `## Workspace: ${name}\n\nThis workspace is being initialized. Follow the hook instructions.\n\n${importBlock}\n`);
    console.log('  Created CLAUDE.md with rule imports');
  } else {
    const existing = readFileSync(claudeMdPath, 'utf-8');
    if (!existing.includes('.workspace-update/')) {
      writeFileSync(claudeMdPath, existing.trimEnd() + '\n\n' + importBlock + '\n');
      console.log('  Added rule imports to CLAUDE.md');
    }
  }
