...
export async function initWorkspace(targetDir) {
  ...
  // Install .claude/ directly from payload (skills, hooks, scripts, agents, rules)
  const payloadClaude = join(payloadDir, '.claude');
  const targetClaude = join(targetDir, '.claude');
  if (existsSync(payloadClaude)) {
    cpSync(payloadClaude, targetClaude, { recursive: true, force: false });
    console.log('  Installed .claude/ (skills, hooks, scripts, agents, rules)');
  }
