...
export async function migrate(targetDir, { update = false } = {}) {
  ...
  console.log('');
  if (has.mcpJson) {
    console.log('  \u26A0 .mcp.json found — review MCP server config manually');
    console.log('    If migrating from Notion: extract content before removing the MCP server');
  }
  if (has.workspaceArtifacts) {
    console.log('  \u26A0 workspace-artifacts/ found — move useful files to .claude-scratchpad/ or shared-context/');
  }
  if (nonStandardDirs.length > 0) {
    console.log(`  \u26A0 Non-standard directories at root: ${nonStandardDirs.join(', ')}`);
    console.log('    Move into repos/ or .claude-scratchpad/ as appropriate');
  }
