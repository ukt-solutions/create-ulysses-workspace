...
- **Existing workspaces won't get this until they `--upgrade`** — by design. The check ships in the template; users on older versions don't have it. Once existing workspaces run `--upgrade` and apply v0.13.0-beta.1 or later, they get the runtime check.
