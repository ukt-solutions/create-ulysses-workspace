...
`shared-context/index.md` is auto-generated by `.claude/scripts/build-shared-context-index.mjs`. It's a one-line catalog of every shared-context file the workspace would actually share — gitignored files (e.g. anything matching `local-only-*`) are excluded automatically, and `shared-context/.indexignore` adds path-prefix excludes for tracked files that shouldn't be in the navigation index (e.g. archived release notes). The script regenerates from frontmatter on demand:
