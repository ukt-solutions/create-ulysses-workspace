...
description: Use when writing, promoting, or organising workspace-context — deciding whether something belongs in auto-memory, team-member, shared, or locked; writing frontmatter for a context file; or regenerating the index, canonical, and per-user catalogs. Carries the frontmatter schema, the generator invocations, and the full reasoning behind the canonical admission test.
---

# Context Placement

Where durable content goes, and how to write it once you've decided. The short decision
table and the canonical test live in `.claude/rules/memory-guidance.md`; this skill is the
detail behind them.
