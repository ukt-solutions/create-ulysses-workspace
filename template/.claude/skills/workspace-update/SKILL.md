...
Apply a staged template update to an initialized workspace. The CLI (`npx @ulysses/create-workspace --upgrade`) stages the payload in `.workspace-update/`. This skill reads and applies it. Runs a maintenance audit before updating and verifies integrity after.

## Prerequisites

- `workspace.json` must have `initialized: true`
- If not initialized, report: "Workspace not initialized. Run /workspace-init first."
- `.workspace-update/` payload directory must exist (staged by `npx @ulysses/create-workspace --upgrade`)
- If no `.workspace-update/` payload exists, report: "No update payload found. Run `npx @ulysses/create-workspace --upgrade` to stage the template."
...
- The CLI (`npx @ulysses/create-workspace --upgrade`) stages the payload. This skill processes it.
...
- Initial setup is handled by `npx @ulysses/create-workspace --init` + `/workspace-init` — this skill is for subsequent updates only
